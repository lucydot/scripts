	...
	if filetype(input_file) == "POSCAR":
		...
		if options.elements:
			...
			if vasp5:
				...
				potcar_lines = subprocess.check_output("grep TITEL POTCAR",bash=True).split("\n")
		...
		atoms = []
		titellines = subprocess.check_output("grep TITEL "+filename,bash=True).split("\n")
		if len(titellines) == 0:
		...
		for line in titellines:
			...
			atoms.append(words[3].split("_")[0])
		# How many of each?
		natoms = [int(s) for s in subprocess.check_output("grep 'ions per type =' "+filename,bash=True).split()[4:]]
